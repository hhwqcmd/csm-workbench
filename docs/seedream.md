...
双向编辑、可复制的完整 cURL、费用确认、结果预览、本地历史和脱敏请求/响应日志。

“复制 cURL”使用当前已应用的图片生成 Method、URL、Headers 和 Request Body。已填写
Key 时命令会包含该 Key，未填写时保留 `<ARK_API_KEY>` 占位符；命令只在浏览器内生成并
写入剪贴板，不进入历史、D1、日志或服务端持久化。
