...
工作台中的七个“填入参数”按钮只更新表单与完整 Request Body，不会自动创建计费任务。
...
任务七填入后会同时展示“生成多个连续视频”面板。它复刻官方三段示例：
...
2026-07-24 使用标准官方 API 逐个提交五项示例，全部达到 `succeeded`：

- 任务一至三使用 Mini。
- 任务四按能力限制使用完整模型。
- 任务五使用 Mini，`web_search` 请求被接受并成功生成结果。
- 刷新页面后仍保留 5 条已完成历史、5 个结果入口和每条任务的创建/状态请求响应日志。

同日新增任务六后使用 Mini 提交，预置人像 Asset ID 被正常接受，任务达到 `succeeded` 并返回结果视频。刷新后验证六条历史均可恢复；任务六包含 1 条创建日志、6 条状态查询日志和结果入口，日志中的 Authorization 保持掩码。

2026-07-27 使用标准官方 API 验证任务七：

- 官方教程女孩首尾帧在创建阶段触发当前真人隐私审核，失败记录和结构化响应被工作台保留。
- 替换为已通过审核的官方插画与连续任务尾帧后，`first_frame` + `last_frame`、`adaptive`、5 秒、`generate_audio: true` 请求达到 `succeeded`，返回有声视频。
- 三段连续任务严格串行完成，三段均达到 `succeeded`，每段都返回视频与 `last_frame_url`；第 2、3 段分别在上一段成功后才创建。

结果视频 URL 属于用户任务产物，不写入仓库。
