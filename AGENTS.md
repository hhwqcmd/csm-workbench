...
- `app/`：演示工作台的产品界面。
  ...
  - 四类模板数据与两个电商可运行案例：`app/lib/template-assets.ts`。
...
- 需要修改模板分类、官方提示词移植或电商可运行案例：读 `docs/template-library.md`。
...
- 模板页加载、分类切换、复制提示词和填入实操台都不能创建真实任务。缺少完整素材的官方模板只允许复制，不得显示为“可运行”。
