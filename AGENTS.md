...
- `app/`：演示工作台的产品界面。
  - 关键入口：`app/page.tsx`。
  - 顶级栏目切换与页面组合：`app/components/WorkspaceShell.tsx`。
  - 双路径选择、任务参数、完整 API 编辑、费用确认、执行、轮询、历史与日志 UI：`app/components/SeedanceTaskRunner.tsx`。
  - 八个官方示例的提示词、素材、参数、模型选择和连续生成计划：`app/lib/seedance-examples.ts`。
  - 示例卡片与“填入参数”入口：`app/components/SeedanceExampleGallery.tsx`。
  - 四类模板数据与十个场景预填案例：`app/lib/template-assets.ts`。
  - 模板分类、复制与“填入实操台”入口：`app/components/TemplateAssetLibrary.tsx`。
...
- 需要修改八个官方示例、首尾帧/连续生成逻辑或判断 Mini / 完整模型能力：读 `docs/official-examples.md`。
- 需要修改模板分类、官方提示词移植或场景预填案例：读 `docs/template-library.md`。
...
- 2026-07-28：把“生成多个连续视频”从任务七的附属入口提升为独立的官方示例任务八，与前七个示例共同排列。任务八的 Request Body 展示第一段真实请求，连续计划仍由同一示例对象交给实操台，避免卡片参数和串行执行逻辑分叉。
- 2026-07-28：新增与演示工作台平级的模板资产库，分为提示词、电商宣发、影视短剧和营销短视频四类。官方内容从 2026-07-20 更新的提示词指南移植；只有两个配齐公开素材的电商案例提供“填入实操台”，并复用原任务执行器、费用确认、历史和日志。
- 2026-07-28：电商模板真实验证中，官方随行杯拼图因宽高比 2.87 超出图片输入 0.40–2.50 被拒绝，故不把该素材保留为可运行案例；替换为已验证的公开果茶商品图后，果茶模板与面霜模板均使用 Mini 达到 `succeeded`。失败记录保留用于演示输入限制和日志诊断。
- 2026-07-28：影视短剧与营销短视频的 8 张卡片全部增加“填入实操台”。具备官方公开素材的请求可直接审核；缺少独立素材的请求保留空 URL 素材位并标记“素材待补”，由任务执行器在补齐前阻止提交。所有入口继续复用费用确认、历史和脱敏日志。
...
- 模板页加载、分类切换、复制提示词和填入实操台都不能创建真实任务。缺少完整素材的官方模板可以预填空素材位，但必须标记“素材待补”，且补齐前不得启用执行按钮。
