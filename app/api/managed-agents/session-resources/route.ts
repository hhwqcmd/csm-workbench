import {
  manageManagedSessionResource,
  ManagedAgentsValidationError,
  parseManageManagedSessionResourceInput,
} from "../../../lib/managed-agents-server";

export async function POST(request: Request): Promise<Response> {
  try {
    const result = await manageManagedSessionResource(
      parseManageManagedSessionResourceInput(await request.json()),
    );
